...
nIdx = 3
flist  = [ ram_dir_path + "run" + str(options.runNumber) + "_ls" + str(options.lumiNumber).zfill(4) + "_index" + str(idx).zfill(6) + ".raw" for idx in range(nIdx*options.subLumiNumber,nIdx*(options.subLumiNumber+1)) ]
print(flist)
...
process.datasets = cms.PSet(
    ...
    DPhysics1    = cms.vstring( 'pPhysics1' )
)

process.streams = cms.PSet(
    SZeroBias = cms.vstring( 'DZeroBias' ),
    SPhysics1    = cms.vstring( 'DPhysics1' )
)
...
process.PrescaleService = cms.Service( "PrescaleService",
    ...
    prescaleTable = cms.VPSet(
      ...
      cms.PSet(  pathName = cms.string( "pPhysics1" ),
...
process.AnalysisMuJet = cms.EDProducer("MuJetAnalysisProducer",
  gmtMuonsTag      = cms.InputTag("GmtUnpacker"),
  caloJetsTag      = cms.InputTag("CaloUnpacker"),
  caloEGammasTag   = cms.InputTag("CaloUnpacker"),
  caloTausTag      = cms.InputTag("CaloUnpacker"),
  caloEtSumsTag    = cms.InputTag("CaloUnpacker"),
  debug = cms.untracked.bool(False)
)

process.AnalysisDispMu2BX = cms.EDProducer("dispMu2BXAnalysisProducer",
  gmtMuonsTag      = cms.InputTag("GmtUnpacker"),
  caloJetsTag      = cms.InputTag("CaloUnpacker"),
  caloEGammasTag   = cms.InputTag("CaloUnpacker"),
  caloTausTag      = cms.InputTag("CaloUnpacker"),
  ...
  maxBx = cms.untracked.int32(3563),
  debug = cms.untracked.bool(False)
)

process.FinalBxSelector = cms.EDProducer("FinalBxSelector",
  gmtMuonsTag      = cms.InputTag("GmtUnpacker"),
  caloJetsTag      = cms.InputTag("CaloUnpacker"),
  caloEGammasTag   = cms.InputTag("CaloUnpacker"),
  caloTausTag      = cms.InputTag("CaloUnpacker"),
  caloEtSumsTag    = cms.InputTag("CaloUnpacker"),
  analysisLabels   = cms.VInputTag("AnalysisMuJet", "AnalysisDispMu2BX"), 
  debug = cms.untracked.bool(False)
)
...
process.prePhysics1 = cms.EDFilter( "HLTPrescaler",
    L1GtReadoutRecordTag = cms.InputTag( "FinalBxSelector" ),
    offset = cms.uint32(0)
)
...
if options.outModule == "root":
    ...
    process.hltOutputSPhysics1 = cms.OutputModule("PoolOutputModule",
        fileName = cms.untracked.string('file:' + ofname + 'Physics1'),
        SelectEvents = cms.untracked.PSet(
            SelectEvents = cms.vstring( 'pPhysics1' )
        ),
        outputCommands = cms.untracked.vstring(
            'drop *',
            #'keep *_GmtUnpacker_*_*',
    ...
    process.hltOutputSPhysics1 = cms.OutputModule( "GlobalEvFOutputModule",
        SelectEvents = cms.untracked.PSet(
            SelectEvents = cms.vstring( 'pPhysics1' )
        ),
        outputCommands = cms.untracked.vstring(
...
process.pPhysics1 = cms.Path(process.rawToDigi + process.AnalysisMuJet + process.AnalysisDispMu2BX + process.FinalBxSelector + process.prePhysics1)


process.ep = cms.EndPath(
    process.hltOutputSZeroBias + process.hltOutputSPhysics1
)
